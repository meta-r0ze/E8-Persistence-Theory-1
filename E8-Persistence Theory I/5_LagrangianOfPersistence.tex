...
\begin{enumerate}
    \item \textbf{Topological Density ($\mathbf{K}$):} Corresponds to the local information density (probability amplitude).
    $$ \mathbf{K}[\Psi] \propto \Psi^\dagger \Psi $$
    \item \textbf{Transition Flux ($\lambda$):} Corresponds to the gauge-covariant rate of state change (gradient energy).
...
No other Lagrangian structure satisfies $\delta S_\Phi = 0$ while respecting the geometric constraints $\{D, \Delta, \nu, \sigma, \chi\}$. The Standard Model is not merely \textit{consistent} with the lattice, it is \textbf{required} by it.