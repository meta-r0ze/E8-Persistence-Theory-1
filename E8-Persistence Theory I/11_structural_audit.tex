...
\begin{align}
\alpha^{-1} &= \ExecuteMetaData[src/results.tex]{AlphaInvVal} \dots \quad (\text{Fixed}) \\
...
The integer invariants $\{\nu=16, D=4\}$ are not tunable. This imposes immediate constraints on new physics searches:
...
\begin{enumerate}
    ...
    \item \textbf{The Supersymmetry Prohibition ($\nu=16$):} The chiral truncation limits the active channel capacity to 16 states per generation. The Standard Model fermions already occupy exactly 48 states ($3 \times 16$). The lattice is saturated.
    \textit{Test:} The discovery of any supersymmetric partner (neutralino, squark, gluino) would imply $\nu > 16$, falsifying the Chiral Rank invariant.
...
\subsection{The Precision $\alpha$ Vector}
The most immediate test is the convergence of the Fine-Structure Constant. The geometric value ($\alpha^{-1}_{geo} = \ExecuteMetaData[src/results.tex]{AlphaInvVal}$) lies between the current electron $g-2$ average and the Cesium recoil measurements.
