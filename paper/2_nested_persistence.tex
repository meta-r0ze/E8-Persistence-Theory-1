...
Any finite information-processing system: a biological cell, an economic network, or a physical substrate, faces a fundamental computing bottleneck. When confronted with complex entropic threats (e.g., keeping its internal parts intact while simultaneously coordinating with the world around it) the state space that a single control loop must regulate expands exponentially. Once this exceeds the system's finite processing capacity, the system's internal model diverges from its environment and it collapses. Systems that persist survive 
