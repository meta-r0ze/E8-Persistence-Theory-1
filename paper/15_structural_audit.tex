...
To verify the theoretical derivation of gravity as a Goldstone mode (Appendix \cref{src:DiffeomorphismAsAnEmergentSymmetry}), we implemented a direct lattice field theory simulation. The code injects a traceless-transverse gravitational wave perturbation $h_{\mu\nu}(x) = \epsilon[e^+_{\mu}e^+_{\nu}\cos(kz) + e^\times_{\mu}e^\times_{\nu}\sin(kz)]$ onto a $16^4$ hypercubic lattice and measures the energy cost via finite-difference gradients.
