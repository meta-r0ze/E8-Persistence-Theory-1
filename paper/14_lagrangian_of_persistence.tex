...
Here, $S_{E_8}$ is the **Informational Action**, defined as the sum of penalties for information loss. This acts as the "Ice Rule" for the substrate: only configurations that preserve information (zero penalty) are permitted to form the macroscopic vacuum.
...
For a structure to persist, its binding energy must exceed the thermal noise floor. This imposes a lower bound on the vacuum impedance ($\alpha^{-1}$) and thus the resonant circumference ($\pi\Delta$).
\begin{equation}
...
\subsection{Transfer-Matrix Calculation of the Beta Function}
...
\subsubsection{Explicit Construction of $\hat{T}(\beta)$}
...
\subsection{Diffeomorphism as an Emergent Symmetry}
...
In lattice loop models (analogous to Spinfoam LQG), the interaction energy of a defect is minimized by minimizing the curvature of the connection. The Informational Action for the metric field is the cost of deviations from the uniform capacity $\nu$:
\begin{equation}
...
While the mode is massless, its stiffness (coupling constant) is determined by the lattice depth. As derived in Section X, the gravitational signal originates from the lattice core (depth $\Delta/2$) and is attenuated by the vacuum impedance. This establishes an effective group velocity scaling $v_g$ relative to the lattice spacing $a$:
\begin{equation}
