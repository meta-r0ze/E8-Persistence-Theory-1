\section{The Attenuation Scale: Gravity and the Planck Mass (\texorpdfstring{$\alpha_G, M_P$}{alphaGMP})} \
...
\textbf{The $E_8$-Persistence Derivation:} Gravity is not a weak force; it is an \textbf{Attenuated Signal}. We derive the gravitational coupling as the Persistence Cost of propagating a signal from the geometric core of the lattice ($\Delta/2$) to the manifold surface. We identify $\alpha_G$ as the signal loss incurred by propagating from the core to the surface, and we derive the Planck Mass not as a fundamental input, but as the \textbf{Unity Threshold}, the mass scale required to overcome this attenuation.

The key insight is geometric: gauge forces originate at the topological boundary (the particle's "surface"), while gravity originates at the lattice centroid (the particle's "core"). The $10^40$ hierarchy is simply the signal attenuation across this geometric depth.
...
\subsection{Residual Capacity (\texorpdfstring{$B_{res}$}{Bres})}
...
\subsection{The Gravitational Coupling (\texorpdfstring{$\alpha_G$}{alphaG})}
...
The experimental dimensionless coupling at the electron scale:
\begin{equation}
\alpha_{G,exp} = \frac{G m_e^2}{\hbar c} \approx \GravCouplingExperimentalValue
\end{equation}
\GravCouplingAccText

\subsection{The Planck Mass (\texorpdfstring{$M_P$}{MP})}
