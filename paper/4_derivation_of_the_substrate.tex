...
Why is the boundary dimension exactly 2? For a particle to exist as a discrete entity (a ``knot") in 3D space, its boundary must be a Closed, Stable Surface that cannot dissolve into the vacuum.
...
\begin{itemize}
    ...
    \begin{equation}
        \int_M K \, dA = 2\pi\chi
    \end{equation}
    \item The unique closed, simply connected surface (a spherical shell) has $\chi=2$. 
    (A Torus has $\chi=0$, indicating neutral curvature. Unlike the sphere, it lacks a preferred ''inside," making it unsuitable as a particle boundary). The unique closed, simply connected surface (a spherical shell, $g=0$) has $X=2$. (Note: A Torus has $g=1$ yielding $X=0$; this vanishing characteristic implies it cannot support a non-zero integral of curvature, rendering it unsuitable for charge storage).
\end{itemize}
