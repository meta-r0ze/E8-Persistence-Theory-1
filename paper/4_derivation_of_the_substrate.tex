...
\subsection{The Metric Signature: Origin of Temporal Cost (\texorpdfstring{$T$}{T}) and Persistent Margin (\texorpdfstring{$PM$}{PM})}
...
\begin{enumerate}
    \item \textbf{Euclidean (4,0):} The algebra is $Cl(4,0) \cong \mathbb{H}(2)$. This supports only real (quaternionic) spinors, which cannot encode the complex phase information required by the Chiral Capacity ($\nu=16$ complex states).
...
\begin{enumerate}
    \item \textbf{The Temporal Cost ($T$): The Negative Eigenvalue ($-1$)}
...
\begin{itemize}
    ...
    \item \textbf{$E_7$ (133 dimensions):} Lacks the necessary triality and complex multiplication properties required to support a three-generation structure of fermions. (Rejected).
...
\begin{enumerate}
    ...
    \item To satisfy the \textbf{Chiral Projection} requirement (Arrow of Time), the symmetry must break to a subgroup supporting complex spinors.
...
\noindent \textbf{The Finite Capacity Constraint:} Standard Quantum Field Theory assumes a vacuum of infinite capacity, leading to divergences. By recognizing the Stabilizing Governor ($G$) as a fundamental requirement of persistence, we impose a physical mechanism that prevents the Energy Vessel from diverging. The Persistence Principle acts as the selection filter, ensuring only geometric configurations with this functioning Governor survive.

\subsubsection{Theorem: Heegner Resonance Uniqueness (\texorpdfstring{$\Delta=43$}{Delta43})}
While the other invariants emerge from the static topology of the projection, the Fundamental Resonance $\Delta$ is a dynamic property that must satisfy three independent conditions for persistence simultaneously. Here we prove that only one integer solution, $\Delta=43$, can satisfy the combined constraints of unitarity, causality, and chemical solvency.

\textbf{Statement:} The $E_8$ lattice admits exactly one Fundamental Resonance $\Delta \in \mathbb{H}$ consistent with a persistent, solvent vacuum containing three generations of fermions. This solution is $\Delta = 43$.
...
By the \textbf{Stark-Heegner Theorem}, there exist exactly nine integers (Heegner Numbers) where $h=1$. This theorem mathematically restricts our search space to the finite set $\{1, 2, 3, 7, 11, 19, 43, 67, 163\}$, ensuring no other solutions exist.
...
\begin{itemize}
    ...
    \item \textbf{The Physical Consequence:} This creates \textbf{Causal Aliasing}. Matter (Left-Chiral) and Mirror (Right-Chiral) signals would collide, destroying the Chiral Projection and breaking time-ordering.
...
\paragraph{Step 3: The Solvency Filter (Chemical Stability)} The geometric impedance $\alpha^{-1}$ is derived geometrically as $\approx \pi\Delta + \chi$. This value dictates the strength of the electromagnetic bond. We test the remaining candidates for physical viability:
...
Having established the local manifold symmetry as $Spin(8)$ and the required chiral capacity as $\nu=16$, we can now trace the symmetry breaking path. The Persistence Principle acts as a filter on the mathematically allowed subgroup chains, which are comprehensively cataloged by Slansky \cite{slansky_group_1981}. We are not free to choose any path; the path must preserve the necessary structures for persistence.

The primary constraint is that the subgroup must support the complex, 16-dimensional spinor representation required by the Chiral Capacity. The maximal subgroups of $Spin(8)$ do not meet this requirement directly. Therefore, the symmetry must first extend to a larger group before breaking. The minimal extension of $Spin(8)$ that contains a 16-dimensional complex spinor is $Spin(10)$.
...
The complete classification of all 248 roots and their physical assignments is provided in Appendix \ref{sec:RootInventoryAndStatePartition}.
...
\]
Only the $\mathbf{16}$ (chiral spinors) is retained as persistent matter; the vector-like and sterile components are filtered out due to high Entropic Action or lack of gauge coupling.
...
\begin{enumerate}
    ...
    \item \textbf{$\Delta=43$}: Fundamental Resonance.
