...
\begin{itemize}
    \item \textbf{The Problem:} If the mathematical field defining the lattice has a ``Class Number" $h > 1$, unique factorization fails (e.g., $6 = 2 \times 3$ but also $6 = a \times b$). The system loses the history of how the state was formed. \textbf{Formal Constraint:} This requirement is rigorously enforced by the \textbf{Unitarity Potential} ($V_U$) in the partition function (\cref{app:partition-function}). Any state with $h > 1$ carries a non-zero entropic penalty and is statistically suppressed in the thermodynamic limit.
...
\begin{enumerate}
    ...
    \item \textbf{The Full Persistence Budget ($H_{full}$):} The total operating cost for a persistent structure. This adds the Spacetime Embedding Cost to the internal channel.
    \begin{itemize}
        \item \textit{The Spinor Tax:} Unlike a vector, a spinor must be rotated by $720^\circ$ (4$\pi$) to return to its original state (the spinor double cover). Consequently, the system must pay the existence cost for each of the $D$ dimensions \textbf{twice} to fully define an embedded state.
    \end{itemize}
    \begin{equation}
        \label{eq:hfull}
...
\noindent \textbf{Note on the Speed of Light:} In this framework, $c$ is not a fundamental velocity, but the emergent \textbf{Channel Capacity Limit} of the substrate, derived from the lattice update rate. (Detailed derivation provided in Paper IV).
