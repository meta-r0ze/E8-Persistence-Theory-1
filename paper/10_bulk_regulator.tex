...
\subsection{System Specification: The Gravity Subsystem}
Gravity acts as a complete persistent system nested within the bulk lattice. We identify its six structural pillars:
...
To maintain persistence, the Gravity Protocol must propagate on the Residual Capacity, the bandwidth left over after the primary gauge allocations are filled.

\subsubsection{1. Residual Capacity ($B_{res}$)}
...
\subsubsection{3. The Gravitational Coupling ($\alpha_G$)}
...
\subsection{Energy Vessel (\texorpdfstring{$\Delta E$}{dE}): The Planck Mass (\texorpdfstring{$M_P$}{MP})}
...
\subsubsection{Temporal Tax (\texorpdfstring{$T$}{T}): The Stiffness of Spacetime}
The \textbf{Temporal Tax} represents the metabolic cost of updating the metric. It defines the "Stiffness" of spacetime against deformation. This tax ($T_{grav}$) scales with the square of the Planck Mass, enforcing the light-cone limit ($ds^2=0$):
\begin{equation}
