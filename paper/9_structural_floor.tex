...
\CatchFileBetweenTags{\HiggsVEVExperimentalValue}{calculations/results.tex}{HiggsVEVExperimentalValue}
\CatchFileBetweenTags{\HiggsVEVAccText}{calculations/results.tex}{HiggsVEVAccText}

\CatchFileBetweenTags{\FermiConstVal}{calculations/results.tex}{FermiConstVal}
\CatchFileBetweenTags{\FermiConstAccText}{calculations/results.tex}{FermiConstAccText}
\CatchFileBetweenTags{\FermiConstExperimentalValue}{calculations/results.tex}{FermiConstExperimentalValue}

\CatchFileBetweenTags{\HiggsMassVal}{calculations/results.tex}{HiggsMassVal}
\CatchFileBetweenTags{\HiggsMassAccText}{calculations/results.tex}{HiggsMassAccText}

\CatchFileBetweenTags{\ElectronYukawaVal}{calculations/results.tex}{ElectronYukawaVal}
\CatchFileBetweenTags{\ElectronYukawaExperimentalValue}{calculations/results.tex}{ElectronYukawaExperimentalValue}
\CatchFileBetweenTags{\ElectronYukawaAccText}{calculations/results.tex}{ElectronYukawaAccText}
...
\begin{equation}
\begin{split}
v_{phys} &= 245.789 \text{ GeV} \times \left(1 + \frac{0.007297}{4}\right) \\
 & \approx \mathbf{\HiggsVEVVal} \text{ GeV}
\end{split}
\end{equation}

\begin{itemize}
    \item \textbf{Geometric Prediction:} $\HiggsVEVVal$ GeV
    \item \textbf{Experimental Value:} $\HiggsVEVExperimentalValue$ GeV
...
Using the derived value $v_{geo} = \HiggsVEVVal$ GeV:
\begin{equation}
...
\begin{equation}
m_H = \sqrt{2 \left(\frac{3}{23}\right)} \cdot (\HiggsVEVVal \text{ GeV})
\end{equation}
\begin{equation}
m_H = \sqrt{0.2608} \cdot \HiggsVEVVal
...
In the $E_8$-Persistence framework, this coupling is not arbitrary; it is the physical manifestation of the Persistence Margin ($PM$) derived in the vacuum impedance equation (Eq. \ref{eq:alpha_inverse}). It represents the smallest non-zero bit of mass the lattice can resolve against thermal noise.

We identify the geometric Yukawa coupling directly with the Persistence Margin.
\begin{equation}
    y_e = PM_{geo} = \frac{1}{H_{full} \cdot (\sigma + 1) \cdot \Delta^2}
\end{equation}

Using the geometric value $PM_{geo} \approx 2.908 \times 10^{-6}$:
\begin{equation}
    y_{e, geo} \approx \ElectronYukawaVal
\end{equation}

\textbf{Comparison to Standard Model:}
In the Standard Model, the Yukawa coupling is related to the electron mass ($m_e$) and vacuum expectation value ($v$) by the Higgs doublet normalization factor $\sqrt{2}$:

\begin{itemize}
    \item \textbf{Standard Model Calculation:} $y_e = \frac{\sqrt{2} m_e}{v} = \frac{1.414 \cdot \MeMeVPrint \text{ MeV}}{246 \text{ GeV}} \approx \ElectronYukawaExperimentalValue$.
    \item \textbf{Accuracy:} \ElectronYukawaAccText
\end{itemize}

\textbf{Physical Interpretation:} The electron mass is not random; it sits exactly at the thermodynamic resolution floor of the $E_8$ lattice ($PM$). The factor of $\sqrt{2}$ in the mass equation ($m_e = v \cdot PM / \sqrt{2}$) represents the geometric aperture of the Weak Force acting on this minimum resolution bit. Any lighter charged particle would have a coupling smaller than $PM$, making it indistinguishable from vacuum fluctuations (massless).
