...
The geometric frustration factor is $\phi^2$ (Golden Ratio squared). However, this projection occurs across the physical manifold, so it must also pay the Manifold Friction tax ($\eta$):
...
\begin{itemize}
    \item \textbf{Why CP violation is weak:} The frustration ($\phi^2 \approx 2.6$) amplifies a tiny Temporal Cost ($10^{-5}$), but the base tax is inherently small due to the efficiency of the weak aperture.
