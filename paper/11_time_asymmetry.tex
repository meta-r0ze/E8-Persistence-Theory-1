...
The Jarlskog Invariant ($J$) quantifies CP violation, the fundamental asymmetry between matter and antimatter (Time directionality). In the $E_8$-Persistence framework, this is not an arbitrary parameter but the Temporal Cost modified by the Projection Frustration of the lattice.

\subsection{The Temporal Cost (\texorpdfstring{$T_{geo}$}{Tgeo})}

In System II (Section V), we identified the \textbf{Temporal Cost} as the entropic cost of state transitions. This represents the minimum energy expenditure required for the system to evolve forward in time.

The cost is derived from the probability of a successful state update within the Weak Interaction aperture ($N=\sigma+1=6$). It scales with the inverse volume of the aperture ($1/N^3$), modulated by the fraction of bandwidth available for interactions ($\chi/\sigma$) and the probability of a successful manifold mapping ($1 - \sigma/D\Delta$).
...
This cost represents the irreversibility of time evolution, the system cannot return to a previous state without paying this entropic cost. It is the geometric origin of the arrow of time.
...
We identify the Jarlskog Invariant as the Temporal Cost ($T_{geo}$) amplified by this Projection Frustration ($\phi^2$). The frustration acts as a geometric amplification factor: every temporal state transition ($T_{geo}$) occurs within the constrained 4D projection, and the $\phi^2$-scaled area deficit accumulates as an additional entropic cost. The product represents the total time-asymmetry penalty:

The geometric frustration factor is $\phi^2$ (Golden Ratio squared). However, this projection occurs across the physical manifold, so it must also pay the Manifold Friction cost ($\eta$):
...
\begin{equation}
J_{geo} = (2.61803 \cdot 0.994186) \cdot (1.185 \times 10^{-5}) \approx \mathbf{\JarlskogVal}
\end{equation}
...
CP violation is not an arbitrary parameter. It is the ``geometric cost" paid for embedding a 5-fold symmetric lattice into a 4-dimensional universe. The frustration of this projection ($\phi^2$) creates the directional bias required for the arrow of time.
...
\begin{itemize}
    \item \textbf{Why CP violation is weak:} The frustration ($\phi^2 \approx 2.6$) amplifies a tiny Temporal Cost ($10^{-5}$), but the base cost is inherently small due to the efficiency of the weak aperture.
