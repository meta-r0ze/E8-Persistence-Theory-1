...
The Weak Mixing Angle represents the relative "weight" of the temporal resonance ($\Delta$) in the total system dynamics. To quantify this, we define the \textbf{Total System Capacity} ($N_{sys}$) as the sum of all structural degrees of freedom in the lattice projection:
...
\begin{enumerate}
    ...
    \item \textbf{Chiral Capacity ($\nu$):} The fermion manifold requires $\nu=16$ independent degrees of freedom to encode the chiral spinor structure (left-handed Weyl fermions). This represents the cost of matter storage.
...
\begin{equation}
N_{sys} = D\Delta + \nu + \sigma = 172 + 16 + 5 = 193
\end{equation}

\subsection{The Partition Formula}

The Weak Mixing Angle is defined as the fraction of the total capacity that is allocated to the temporal resonance (the "clock frequency" $\Delta$ of the lattice):

\begin{equation}
\sin^2 \theta_W = \frac{\text{Resonant Frequency}}{\text{Total Capacity}} = \frac{\Delta}{D\Delta + \nu + \sigma}
\end{equation}
...
\begin{equation}
\sin^2 \theta_W = \frac{43}{193} \approx \mathbf{\WeakAngleVal}
\end{equation}
...
The geometric validity of the Weak Mixing Angle is reinforced by a structural link back to the Geometric Impedance derived in \cref{sec:Vacuum_Impedance}.

Recall the \textbf{Temporal Tax} ($T$), the Entropic Action of a state transition defined purely by lattice integers in Eq. \ref{eq:alpha_inverse}:
