...
\CatchFileBetweenTags{\AlphaSAccText}{constants.tex}{AlphaSAccText}
...
\CatchFileBetweenTags{\AlphaRunningAccText}{constants.tex}{AlphaRunningAccText}
...
\begin{enumerate}
    \item \textbf{Chiral Saturation ($\nu \cdot \eta$):} A confined state requires the synchronization of the full chiral rank ($\nu=16$) to prevent information leakage. As a bulk capacity, it is strictly subject to the The Coordinate Overhead ($\eta$).
...
Fermions screen the charge by distributing the boundary topology ($\chi=2$) across the generation manifold ($n_{gen}=3$, derived strictly from the Triality of the $D_4$ internal sector in \textit{Informational Energetics: A Universal Architecture of Persistence}\cite{meyer_informational_2026}) is geometrically isomorphic to the available degrees of freedom in the internal color bulk ($\sigma - \chi = 5 - 2 = 3$). Distributing the boundary evenly across these internal dimensions yields the exact fractional screening per flavor:
\begin{equation}
...
This form, functionally identical to the perturbative Renormalization Group Equation, emerges here as the linear response of the lattice stiffness ($\beta_0$) to scale transformations.

\textit{Note on the Active Degrees of Freedom:} Standard perturbative QCD requires manually patching the RGE across quark mass thresholds (e.g., shifting from $n_f=5$ at $M_Z$ down to $n_f=3$ at $M_\tau$). In the $E_8$-Persistence theory, the global lattice stiffness ($\beta_0$) is a topological invariant of the bulk, driven by the $n_{gen}=3$ generation manifold rather than local kinematic mass thresholds. Therefore, the global geometric scaling utilizes the structural base state ($n_f = 3 \implies \beta_0 = 9$) across the entire macroscopic resolution interval. 
...
\textbf{Numerical Proof (The Unitary Resonance):}
To test the QED screening effect at the Z-Pole ($M_Z$), we adopt the experimentally evaluated vacuum polarization ``Screening Fog'' $\Delta \alpha_{\text{total}} \approx 0.0590$ (PDG 2024) as an external input; deriving this quantity from the lattice requires integrating over the full particle spectrum (the subject of future work). Applying this shift to the static vacuum impedance yields a standard effective coupling of $\alpha^{-1}_{QFT} \approx 137.036 - 8.085 = 128.951$ (where the absolute shift is $\alpha^{-1}_{geo} \cdot \Delta \alpha_{\text{total}} \approx 8.085$). This leaves a glaring $\approx 1.0$ discrepancy against the experimental value of $127.955$.

The $E_8$-Persistence framework explicitly resolves this gap. At the exact energy of the Z-Pole ($M_Z$), the vacuum undergoes a \textbf{Resonant Transition}, coupling to the Scalar Ground State ($\Delta^0 = 1$). This unitary geometric step is projected onto the physical manifold and is therefore subjected to the The Coordinate Overhead ($\eta$). The true geometric impedance is:
...
\textbf{Physical Interpretation:} The precision of this match confirms that the difference between the low-energy coupling ($\sim 137$) and the Z-pole coupling ($\sim 128$) is not arbitrary. It is the exact sum of standard fermion screening (QFT) and exactly one unit of geometric resonance ($\Delta^0=1$), corrected for manifold projection efficiency ($\eta$).

\textit{Scope Defense:} We utilize the experimental value of $\Delta\alpha_{total}$ strictly to isolate and validate the geometric projection overhead ($\eta$) at the Z-Pole. The ab initio derivation of the full fermion spectrum required to calculate $\Delta\alpha_{total}$ purely from geometry is the subject of a companion paper. This temporary empirical anchor does not introduce a free parameter into the fundamental architecture.
...
This derivation demonstrates that the Casimir invariants of the Standard Model are not fundamental inputs, but are the \textbf{algebraic projections} of the underlying lattice topology. The successful prediction of QED screening ($4/3$) from identical geometric principles confirms this is not a coincidence, but a fundamental property of embedding topological charges in 4-dimensional spacetime.
