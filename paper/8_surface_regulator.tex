\section{The Resonant Interface (Electroweak Scale) (\texorpdfstring{$v, \lambda, m_H, y_e$}{vlambdamhye})}
\label{sec:surface_regulator}

\CatchFileBetweenTags{\AlphaInvVal}{constants.tex}{AlphaInvVal}
\CatchFileBetweenTags{\CompPM}{constants.tex}{CompPM}
...
\begin{itemize}
    \item \textbf{Capacity (CAP): The Vacuum Expectation Value ($v$).} \\
    ...
    \item \textbf{Identity (MAP): Scalar Hypercharge ($Y=1$).} \\
...
The VEV is derived from the Lattice Resonance ($\Delta$) scaled by the structural overhead ($L_{substrate}$) and the vacuum impedance ($\alpha^{-1}$).
\begin{equation}
v_{geo} = (\chi \Delta^2 - L_{substrate}) \cdot \alpha^{-1} \cdot m_e 
\end{equation}
Substituting the invariants ($L_{substrate} = 188$):
...
The physical vacuum is not a static abstract background; it is a topological manifold subject to thermodynamic fluctuations. To bridge the ideal geometric floor ($v_{geo}$) to physical reality, we identify two independent correction factors mandated by the lattice projection.
...
This creates a geometric dilution factor based on the vacuum coupling:
\begin{equation}
\delta_{topo} = \left( 1 + \frac{\alpha}{D_{eff}} \right)
\end{equation}

\paragraph{Thermodynamic Noise (Temporal Stability)}
We must also account for the \textbf{Persistence Margin} (MAR). Since the vacuum stability floor supports $n_{gen}=3$ generations ($\sigma - \chi = 3$), the noise amplitude per generation is partitioned linearly. This creates a thermodynamic reduction factor:

\begin{equation}
\delta_{noise} = \left( 1 - \frac{PM}{\sigma - \chi} \right)
\end{equation}

\begin{equation}
PM = \frac{1}{L_{embed} \cdot (\sigma + 1) \cdot \Delta^2} = \frac{1}{31 \cdot 6 \cdot 43^2} \approx \CompPM
\end{equation}
...
\begin{equation}
\begin{split}
v_{phys} &= (\HiggsVEVStepOneVal) \\
& \cdot \left( 1 + \frac{\alpha}{4.159} \right) \cdot \left( 1 - \frac{\ElectronYukawaStepOneVal}{3} \right) \\
& \approx \textbf{$\HiggsVEVVal$}
\end{split}
\end{equation}
...
In the Standard Model, the effective 4-fermion weak interaction cross-section contains a $1/\sqrt{2}$ normalization factor arising from the algebraic definition of the $SU(2)_L$ currents. In the $E_8$-Persistence framework, this is not an algebraic convention, but a strict geometric projection.

The weak interaction mediates flux across the 2D spherical topological boundary ($\chi=2$) of the node. For this 2D boundary flux to interact with a 1D chiral fermion trajectory, the flux must be projected onto the 1D interaction axis. By Pythagorean normalization across the two orthogonal basis vectors of the boundary, the geometric cross-section is attenuated by exactly $\sqrt{\chi} = \sqrt{2}$. 

Therefore, the Fermi constant ($G_F$) is precisely the inverse squared VEV attenuated by this topological flux projection:
\begin{equation}
    G_F = \frac{1}{\sqrt{\chi}v_{phys}^2} \approx \mathbf{\FermiConstVal}
\end{equation}
...
\subsubsection{The Systemic Channel (\texorpdfstring{$L_{intrinsic}$}{Hsys})}
...
\begin{equation}
\lambda = \frac{(\sigma - \chi) - 1/\Delta}{L_{intrinsic}} = \frac{2.976744}{23} \approx \mathbf{\HiggsLambdaVal}
\end{equation}
\begin{itemize}
...
The Persistence Margin (MAR) represents the smallest non-zero bit of mass the lattice can resolve against thermal noise. This determines the coupling of the lightest charged particle (the Electron). We derive this scale as the Unit Bit (1) diluted over the \textbf{Total Configuration Space Volume}.

\begin{equation}
y_{e,bare} = \frac{1}{V_{config}} = \frac{1}{\underbrace{L_{embed}}_{\text{Budget}} \cdot \underbrace{(\sigma+1)}_{\text{Aperture}} \cdot \underbrace{\Delta^2}_{\text{Area}}}
\end{equation}

\subsubsection{Physical Interpretation: The Landauer Limit}
Physically, the electron represents the \textbf{Landauer Limit} of the vacuum geometry. Any topological knot with a coupling $y < y_{e,bare}$ would possess a binding energy lower than the thermal noise floor of the substrate ($E < k_B T_{eff} \ln 2$), making it informationally indistinguishable from the vacuum fluctuations.

The electron is stable because it is the \textbf{Minimum Viable Particle}—the physical instantiation of a single bit of mass.
...
A charged particle possesses an interaction structure of rank $\sigma=5$ (from System 1), but it must exist within a manifold of rank $D=4$. This dimensional mismatch ($\sigma > D$) creates an irreducible ``Overpressure" on the surrounding lattice.
...
\begin{enumerate}
    ...
    \item \textbf{Manifold  Quantization Efficiency cost ($\eta$):} This projection occurs across the physical manifold, so it must also pay the Manifold  Quantization Efficiency cost ($\eta$)
